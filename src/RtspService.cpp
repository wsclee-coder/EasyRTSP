...
//#define DEVICE_VIDEO "/dev/video1"


//#define BITSTREAM_LEN (1280*720*3/2)
#define BITSTREAM_LEN (640*480*3/2)
...
static int init_camera(int fd)
{
	...
	/* Buggy driver paranoia. */
        unsigned int min = tv_fmt.fmt.pix.width * 2;
        if (tv_fmt.fmt.pix.bytesperline < min)
                tv_fmt.fmt.pix.bytesperline = min;
        min = tv_fmt.fmt.pix.bytesperline * tv_fmt.fmt.pix.height;
        if (tv_fmt.fmt.pix.sizeimage < min)
                tv_fmt.fmt.pix.sizeimage = min;
...
static void open_camera(void)
{
	...
	CameraDeviceFd = open(DEVICE_VIDEO, O_RDWR | O_NONBLOCK,0);
	if(CameraDeviceFd < 0)
	{
...
:m_nEncType(RTSP_MEDIA_TYPE_MJPEG)
,m_bRunning(false)
...
void RtspService::OnRun()
{
	...
	while(m_bRunning)
	{
		// no timeout after putting below inside the loop
		...
		ret = select(CameraDeviceFd + 1, &fds, NULL, NULL, NULL);// no timeout is less lagging
		...
		OnRtspJPEGData((unsigned char *)usr_buf[buf.index].start, usr_buf[buf.index].length, 0, pts);
